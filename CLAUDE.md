...
Layout: `src/logic/` holds the ported logic (`dictionary-key`, `string-utils`, `front-coding`, `dictionary` = Dictionary + MapDictionary, `base-reader`, `grille`, `playfair`, `number-analysis` with `bigint` for ULong, `azimuth`, `holiday`, `format`), `src/shell/` the app shell (`router` with `#/<tool>` hashes and the `Tool` interface, `dom` with the `h()` element builder, `toast`, `storage` = localStorage counterpart of SharedPreferences, `icons` generated from the app's vector drawables), `src/components/` reusable pieces (`endless-rows`, `cell-grid`, `map-view`, `map-picker`), `src/tools/` one module per screen, `src/workers/search-worker.ts` the search off the main thread. `public/assets/` (Git LFS) holds the front coded `.cbfcdict`/`.cbfcmap` files made by `utils/`, `public/legend/` the reader legend images copied from the app's drawables. Saved state uses the same keys as the app's SharedPreferences, in `localStorage` per tool. The Android app does not use anything from here.
